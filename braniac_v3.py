...
from math_utils import dist, fleet_speed, point_to_segment_distance, segment_hits_sun, launch_point
...
EVAL_WEIGHTS = {
    ...
    "enemy_leader_ships": -0.5,
    "we_are_leader_penalty": -50.0,
    "my_spread_penalty": -0.5,
...
def evaluate_board(state_dict, in_flight_counts, world, my_player_id):
    ...
    if we_are_leader:
        score += EVAL_WEIGHTS["we_are_leader_penalty"]
        
    score += my_spread * EVAL_WEIGHTS["my_spread_penalty"]
    score += enemy_spread * EVAL_WEIGHTS["enemy_spread_bonus"]
...
def fast_score_action_via_delta(
    ...
    for t in range(1, max_h + 1):
        ...
        if t == 30 or t == 60 or t == 100:
            ...
            if we_are_leader:
                score += eval_weights[6]
            score += my_spread * eval_weights[7]
            score += enemy_spread * eval_weights[8]
            
            if t == 30:
                total_score += score * eval_weights[9]
            elif t == 60:
...
def prepare_world_state(world):
    ...
    eval_weights = np.array([
        EVAL_WEIGHTS["my_ships"],
        EVAL_WEIGHTS["my_production"],
        EVAL_WEIGHTS["my_planets"],
        EVAL_WEIGHTS["enemy_ships_total"],
        EVAL_WEIGHTS["enemy_prod_total"],
        EVAL_WEIGHTS["enemy_leader_ships"],
        EVAL_WEIGHTS["we_are_leader_penalty"],
        EVAL_WEIGHTS["my_spread_penalty"],
        EVAL_WEIGHTS["enemy_spread_bonus"],
        EVAL_WEIGHTS["weight_t30"],
        EVAL_WEIGHTS["weight_t60"],
        EVAL_WEIGHTS["weight_t100"]
    ], dtype=np.float64)
...
def generate_candidate_missions(world, available):
    ...
    for src in world.my_planets:
        ...
        for tgt in world.planets:
            ...
            score = turns * CANDIDATE_WEIGHTS["dist_penalty"]
