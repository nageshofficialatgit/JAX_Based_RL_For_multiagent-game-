...
from env_jax.orbit_env import EnvState, step_physics, apply_actions, build_observation, reset_env
...
GAMMA = 0.999
GAE_LAMBDA = 0.95
CLIP_EPS = 0.2  # MAPPO standard clip
VF_COEF = 0.5
MAX_POOL_SIZE = 50
...
def train_ppo():
    ...
    bc_ckpt_dir = os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'checkpoints/bc_light'))
    # Save PPO weights to a separate ppo_light directory
    ppo_ckpt_dir = os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'checkpoints/ppo_light'))
    ...
    if not restored:
        try:
            bc_bin_path = os.path.abspath(os.path.join(ppo_ckpt_dir, "../benchmarks/bc_light_60000.bin"))
            if os.path.exists(bc_bin_path):
                print(f"Loading BC weights from {bc_bin_path}...")
                ...
                with open(bc_bin_path, "rb") as f:
                    raw_bytes = f.read()
                
                pure_state = force_to_pure_dict(state)
                restored_model_dict = serialization.from_bytes(pure_state, raw_bytes)
                
                restored_model_casted = jax.tree_util.tree_map(
                    lambda r, t: jnp.asarray(r, dtype=t.dtype),
                    restored_model_dict, pure_state
                )
                nnx.update(model, restored_model_casted)
                print(f"BC weights loaded successfully.")
                start_iteration = 0
                restored = True
            else:
                print(f"Could not find {bc_bin_path}")
        except Exception as e:
            print(f"Failed to load BC bin: {e}")
    ...
    def env_step(env_state, rng_key, merged_model, opponent_model, reward_config):
        ...
        _, l_logits1, a_logits1, s_logits1, ppo_value, l_act1, target_act1, s_act1 = merged_model(
        ...
        _, l_logits_opps, a_logits_opps, s_logits_opps, _, l_act_opps, target_act_opps, s_act_opps = opponent_model(
    ...
    def compute_advantages(transitions, final_value, ema_mean, ema_var):
        ...
        dataset = {
            'obs': transitions['obs'].reshape(-1, 70, 14),
    ...
    def loss_fn(s_flat, batch, current_entropy, current_clip):
        ...
        valid_launch_mask = p1_mask & (batch['obs'][:, 1:51, 6] >= (jnp.log1p(1.0)/7.0 - 1e-4))
        v_logits, launch_logits, angle_logits, ships_logits, ppo_v = merged(
            batch['obs'], return_policy=True, target_launch=batch['a_launch'], target_angle=batch['a_angle'], valid_launch_mask=valid_launch_mask)
        ...
        new_lp = (launch_lp + jnp.where(batch['a_launch'], angle_lp + ships_lp, 0.0)) * p1_mask
        ...
        surr1 = jnp.where(p1_mask, ratio * adv_broadcast, 0.0)
        surr2 = jnp.where(p1_mask, jnp.clip(ratio, 1.0 - current_clip, 1.0 + current_clip) * adv_broadcast, 0.0)
        
        policy_loss = -jnp.sum(jnp.minimum(surr1, surr2)) / (jnp.sum(p1_mask) + 1e-8)
        ...
        angle_probs = jax.nn.softmax(angle_logits, axis=-1)  
        target_ent_raw = -jnp.sum(angle_probs * jnp.log(angle_probs + 1e-8), axis=-1)
        
        ships_probs = jax.nn.softmax(ships_logits, axis=-1) 
        ships_ent_raw = -jnp.sum(ships_probs * jnp.log(ships_probs + 1e-8), axis=-1)
        
        # FIX: Mask unused head entropy by launch_prob to prevent entropy farming
        valid_entropies = (launch_ent_raw + launch_prob * (0.5 * target_ent_raw + 0.3 * ships_ent_raw)) * p1_mask
        entropy = jnp.mean(jnp.sum(valid_entropies, axis=1))
        ...
        metrics = {
            ...
            'clip_frac': jnp.mean(jnp.abs(ratio - 1.0) > current_clip),
    ...
    model_rand = EntityTransformer(num_features=14, num_classes=5, rngs=nnx.Rngs(999))
    ...
    for iteration in range(start_iteration, start_iteration + 10000):
        ...
        shock_progress = min(1.0, max(0.0, iteration / 2000.0))
        target_entropy = 4.0 - (3.5 * shock_progress) # Decays target from 4.0 to 0.5
